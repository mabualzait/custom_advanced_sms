...
import com.abualzait.custom_advanced_sms.permisions.Permissions
import com.abualzait.custom_advanced_sms.status.SmsStateHandler
 ...
 * Updated by abualzait for custom namespace
 */

/** CustomAdvancedSmsPlugin */
class CustomAdvancedSmsPlugin() : FlutterPlugin, ActivityAware {
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    this.receiveSmsChannel = EventChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/recvSMS", JSONMethodCodec.INSTANCE)

    this.smsStatusChannel = EventChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/statusSMS", JSONMethodCodec.INSTANCE)

    this.sendSmsChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/sendSMS", JSONMethodCodec.INSTANCE)

    this.removeSmsChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "abualzait.sms.remove.channel")

    this.querySmsChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/querySMS", JSONMethodCodec.INSTANCE)

    this.queryContactChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/queryContact", JSONMethodCodec.INSTANCE)

    this.queryContactPhotoChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/queryContactPhoto", StandardMethodCodec.INSTANCE)

    this.userProfileChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/userProfile", JSONMethodCodec.INSTANCE)

    this.simCardChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "plugins.abualzait.com/simCards", JSONMethodCodec.INSTANCE)
